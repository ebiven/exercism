// This problem is just an exercise in typing, so I copied the example solution
// instead of wearing out my keyboard.
...
`;
   ...
   */
  verses(first, last) {
    let rangeLength = last - first + 1;
    let sequence = Array.from({ length: rangeLength }, (v, k) => first + k); // integers from first to last

    // build the final string
    let str = sequence.map(x => this.verse(x));
    str.push('');
    return str.join('\n');
  }
